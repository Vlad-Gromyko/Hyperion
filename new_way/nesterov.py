...
class GSplusWeight(Experiment):
    ...
    def run(self, iterations):
        ...
        for k in range(iterations):
            x = weights + velocity * self.gradient(weights)

            weights = x_list[-1] - momentum * (x - x_list[-1])
            x_list.append(x)
            self.iteration(weights)
...
if __name__ == '__main__':
    ...
    exp.add_array(0 * UM, 0, 160 * UM, 160 * UM, 3, 3)
